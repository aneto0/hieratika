...
//Constructor
export class HtkDiscreteCoils2D extends HTMLElement {
    constructor() {
        super();
    }
    ...
    variablesInfoLoaded() {
        ...
        if (coilVariables !== undefined) {
            ...
            $.each(coilVariables, function (i, coilVariable) {
                var coil = {color:Constants.STANDARD_FCOLOR};
                var coilRef = {color:Constants.REFERENCE_COLOR};
                var coilPlant = {color:Constants.PLANT_COLOR};
    ...
    valueChanged(src, typeOfChange) {
        ...
        else if (typeOfChange === Constants.VALUE_CHANGED_PLANT) {
            coil = this.coils[coilId + "Plant"];
            value = parseFloat(src.getPlantValue());
        }
        else if (typeOfChange === Constants.VALUE_CHANGED_REFERENCE) {
    ...
    setCoilHighlighted(coilId, highlight) {
        ...
        if (coil !== undefined) {
            if (highlight) {
                ...
                var color = Constants.STANDARD_FCOLOR;
                coil.line.setAttribute("stroke", color);
                coil.text.setAttribute("fill", color);
    ...
    drawVessel(w, color) {
        ...
        this.canvas.appendChild(this.createLine(this.getX(rcoords[rcoords.length - 1]), this.getY(zcoords[zcoords.length - 1]), this.getX(rcoords[0]), this.getY(zcoords[0]), w, color));
    }
}
window.customElements.define('htk-discrete-coils-2d', HtkDiscreteCoils2D);
