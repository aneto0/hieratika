...
class PMCTestStruct2 extends PMCVariable {
    ...
    public PMCTestStruct2(String name, String description, boolean isLiveVariable, PMCTestStruct1[] var2) {
        ...
        for(int i=0; i<this.var3.length; i++) {
            for(int j=0; j<this.var3[i].length; j++) {
                for(int k=0; k<this.var3[i][j].length; k++) {
                    this.var3[i][j][k] = new PMCTestStruct1("var3_" + (i * this.var3[i].length * this.var3[i][j].length + j * this.var3[i][j].length + k), "Test gap", false, i, j, k, 0);
                }
            }
        }
        this.var1 = new PMCTestStruct1("var1", "Another struct", false, 1, 2, 3, 4);
        this.var4 = new PMCVariable("var4", "A variable", new Vector(Arrays.asList(new int[]{1})), "int32", new int[]{1}, false, false, 
            new PMCValidation[]{
