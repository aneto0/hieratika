...
export class HtkHelper {
    ...
    displayPlant() {
        var mainFrameHtkComponents = this.htkMainEditor.getAllMainFrameHtkComponents();
        if ((mainFrameHtkComponents !== null) && (mainFrameHtkComponents !== undefined)) {
            var keys = Object.keys(mainFrameHtkComponents);
            for (var i = 0; i < keys.length; i++) {
                var htkCompArray = mainFrameHtkComponents[keys[i]];
                for (var j = 0; j < htkCompArray.length; j++) {
    ...
    /**
     * @brief Note that the main page must have included:
     * <script type="text/javascript" src="../js/math.js"></script>
     * Injecting this call will not work as the script is loaded asynchronously and it is not obvious on how to synchronise without changing the architecture.
     * A much better alternative would be include math.js as an external module, but that didn't work as well.
     */
    static getMathLibrary() {
        var m = window.math;
        if (m === undefined) {
            //Handle iframes
            m = parent.math;
