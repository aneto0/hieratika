...
import { HtkDialogs } from './htk-dialogs.js'
        ...
        export class HtkHelper {
                    ...
                    success: function (response) {
                        if (this.checkServerResponse(response)) {
                            if (response === constants.HTK_IN_USE) {
                                parent.htkDialogs.showErrorDialog("The schedule is being used in other schedules and thus cannot be overwritten (note that it might being used by this same variable. If so unselect if first).");
                                errorFun();
                            }
                            else {
                                successFun();
                        ...
                        success: function (response) {
                            if (this.checkServerResponse(response)) {
                                if (response === constants.HTK_OK) {
                                    ...
                                    parent.htkDialogs.showErrorDialog("Unkown error while deleting the folder " + folderName + " in the server. Is the folder empty?. Check the logs.");
                                    errorFun();
                                }
                            }
                        }.bind(this),
                        ...
                        success: function (response) {
                            if (this.checkServerResponse(response)) {
                                if (response === constants.HTK_OK) {
                                    ...
                                    parent.htkDialogs.showErrorDialog("Unkown error while obsoleting the folder " + folderName + " in the server. Check the server logs.");
                                    errorFun();
                                }
                            }
                        }.bind(this),
            ...
            saveLibrary(libraryType, libraryName, libraryDescription, variables, successFun, errorFun) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        if(this.checkServerResponse(response)) {
                            if (response === HTK_IN_USE) {
                                parent.htkDialogs.showErrorDialog("Library is being used in other schedules and thus cannot be overwritten (note that it might being used by this same variable. If so unselect if first).");
                                errorFun();
                            }
                            else if (response === HTK_UNKNOWN_ERROR) {
                                parent.htkDialogs.showErrorDialog("Unkown error while saving the library in the server");
                                errorFun();
                            }
                            else {
                        ...
                        success: function (response) {
                            if(this.checkServerResponse(response)) {
                                if (response === HTK_OK) {
                                    ...
                                    if (response === HTK_IN_USE) {
                                        parent.htkDialogs.showErrorDialog("Schedule " + scheduleUID + " is being used in other schedules and thus cannot be deleted.");
                                        errorFun();
                                    }
                                    else if (response === HTK_NOT_FOUND) {
                                        parent.htkDialogs.showErrorDialog("Schedule " + scheduleUID + " could not be found in the server!");
                                        errorFun();
                                    }
                                    else {
                                        parent.htkDialogs.showErrorDialog("Unkown error while deleting the schedule " + scheduleUID + " in the server");
                                        errorFun();
                                    }
                                }
                            }
                        ...
                        success: function (response) {
                            if(this.checkServerResponse(response)) {
                                if (response === HTK_OK) {
                                    ...
                                    if (response === HTK_NOT_FOUND) {
                                        parent.htkDialogs.showErrorDialog("Schedule " + scheduleUID + " could not be found in the server!");
                                        errorFun();
                                    }
                                    else {
                                        parent.htkDialogs.showErrorDialog("Unkown error while obsoleting the schedule " + scheduleUID + " in the server");
                                        errorFun();
                                    }
                                }
                            }
                    ...
                    success: function (response) {
                        if(this.checkServerResponse(response)) {
                            if (response === HTK_OK) {
                                ...
                                if (response === HTK_IN_USE) {
                                    parent.htkDialogs.showErrorDialog("Library is being used in other schedules and thus cannot be deleted.");
                                    errorFun();
                                }
                                else if (response === HTK_NOT_FOUND) {
                                    parent.htkDialogs.showErrorDialog("Library could not be found in the server!");
                                    errorFun();
                                }
                                else {
                                    parent.htkDialogs.showErrorDialog("Unkown error while deleting the library in the server");
                                    errorFun();
                                }
                            }
                        }
                        ...
                        if(this.checkServerResponse(response)) {
                            if (response === HTK_OK) {
                                ...
                                if (response === HTK_NOT_FOUND) {
                                    ...
                                    parent.htkDialogs.showErrorDialog("Unkown error while obsoleting the library in the server");
                                    errorFun();
                                }
                            }
                        }
            ...
            showCriticalError(err) {
                parent.htkDialogs.showErrorDialog("Critical error communicating with the server." + err);
            }
        ...
        htkHelper = undefined;
        if (htkHelper === undefined) {
            htkHelper = new HtkHelper();
            setInterval(htkHelper.synchroniseRemote, 2000);
        }
